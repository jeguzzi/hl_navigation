...
import h5py
import numpy as np
from hl_navigation import Pose2
...
class Run:
    ...
    def __init__(self,
        ...
        self._max_step = len(self.trace) - 1
        self._step = 0
        self.index = index
        p1 = np.min(trace, axis=(0, 1))[:2]
        p2 = np.max(trace, axis=(0, 1))[:2]
        self.bounds = (p1, p2)

    def do_step(self):
        if self._step < self._max_step:
            # self.world.time += self.time_step
            for ps, agent in zip(self.trace[self._step], self.world.agents):
