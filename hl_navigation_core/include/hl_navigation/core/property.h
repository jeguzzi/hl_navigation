#ifndef HL_NAVIGATION_CORE_PROPERTY_H
#define HL_NAVIGATION_CORE_PROPERTY_H
 ...
 * hl_navigation::core::Vector2 or a collection (a vector in C++, a list in Python) thereof.
 * Properties are used to configure sub-classes of \ref hl_navigation::core::HasProperties when using
 * bindings like YAML or Python. Properties values are accessed 
 * using the methods exposed by \ref hl_navigation::core::HasProperties.
 */
struct Property {
  /**
 ...
 */
struct HL_NAVIGATION_CORE_EXPORT HasProperties {
...
inline std::ostream& operator<<(std::ostream& os,
                                const hl_navigation::core::Property::Field& value) {
  std::visit([&os](auto&& arg) { os << arg; }, value);
  return os;
}

#endif  // HL_NAVIGATION_CORE_PROPERTY_H
