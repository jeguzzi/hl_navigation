...
class Thymio(pyenki.Thymio2):
    ...
    targets: Iterable[Tuple[Tuple[float, float, float], Tuple[float, float]]]
    thymios: Set['Thymio']
...
def main() -> None:
    ...
    if arg.rt:
        world.run_in_viewer(cam_position=(100, 100), cam_altitude=300.0, cam_yaw=0.0,
                            cam_pitch=-1, walls_height=10,
                            orthographic=False, realtime_factor=2.0)
    else:
