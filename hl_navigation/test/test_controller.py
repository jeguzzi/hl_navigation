...
def main(behavior_name: str = "HL") -> None:
    agent = hl_navigation_py.agent_with_name(
        behavior_name, hl_navigation_py.AgentType.HOLONOMIC, 0.1, 0.2)
    if not agent:
    ...
    #     [hl_navigation_py.Disc(position=hl_navigation_py.CVector2(0.5, 0.0), radius=0.1)])
    ...
    t = 0.0
    while controller.state != hl_navigation_py.ControllerState.IDLE and t < 2:
        controller.update(dt)
        agent.velocity = agent.target_velocity
        agent.position += agent.velocity * dt
        agent.angle += hl_navigation_py.CRadians(agent.target_twist.angular * dt)
