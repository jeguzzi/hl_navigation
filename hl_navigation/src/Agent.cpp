...
void Agent::setDesiredWheelSpeeds(double left,double right)
{
    ...
    desiredAngularSpeed=CRadians((rightWheelDesiredSpeed-leftWheelDesiredSpeed)*0.5/axisLength);
...
void Agent::setRotationTau(double value)
{
  rotationTau=value;
}
void Agent::setSafetyMargin(double value)
{
  safetyMargin=value;
}

void Agent::setHorizon(double value)
{
  horizon=value;
}

void Agent::setMaxSpeed(double value)
{
  maxSpeed=value;
}
...
void Agent::updateVelocity()
{
    ...
    if(type==TWO_WHEELED)
    {
        ...
        Real targetAngularMotorSpeed=desiredAngularSpeed.GetValue()*0.5*axisLength;

        if(targetAngularMotorSpeed>optimalRotationSpeed)
        {
            targetAngularMotorSpeed=optimalRotationSpeed;
            desiredAngularSpeed=CRadians(targetAngularMotorSpeed*2.0/axisLength);
        }
        ...
        else
        {
            ...
            {
                desiredLinearSpeed=desiredSpeed*(1-fabs(targetAngularMotorSpeed)/maxRotationSpeed);
            }
            else
            {
...
Real Agent::marginForObstacleAtDistance(Real distance,Real obstacleRadius,Real safetyMargin,Real socialMargin)
{
  ...
  //socialMargin is now a property of the obstacle!!, so could be less than safety margin
