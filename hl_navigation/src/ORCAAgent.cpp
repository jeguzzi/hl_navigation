...
void ORCAAgent::setTimeHorizon(double value){
timeHorizon=value;
}
...
void ORCAAgent::updateVelocity()
{
    ...
    {
        setDesiredWheelSpeeds(desiredSpeed*(Cos(desiredAngle)-axisLength*0.5/D*Sin(desiredAngle)),desiredSpeed*(Cos(desiredAngle)+axisLength*0.5/D*Sin(desiredAngle))); 
    }
    else
    {
...
void ORCAAgent::addObstacleAtPoint(CVector2 p,CVector2 v,Real r,Real socialMargin)
{
