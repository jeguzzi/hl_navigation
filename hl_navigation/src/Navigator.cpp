...
template <typename T> bool Navigator::inOdomFrame(const T &in, T &out) {
  try {
...
void Navigator::setTargetFromPointMessage(
    const geometry_msgs::msg::PointStamped &msg) {
  setTargetFromPoint(msg);
}

bool Navigator::setTargetFromPoint(
    const geometry_msgs::msg::PointStamped &msg) {
  if (odom_frame == "") {
  ...
  RCLCPP_INFO(this->get_logger(), "Set target point to (%.3f, %.3f)",
              targetPoint.point.x, targetPoint.point.y);
  state = MOVE;
  if (drawing_enabled)
    drawTarget(targetPoint.point, targetPoint.header.frame_id);
  return true;
}

void Navigator::setTargetFromPoseMessage(
...
bool Navigator::setTargetFromPose(const geometry_msgs::msg::PoseStamped &msg) {
  ...
  agent->targetPosition =
      CVector2(targetPose.pose.position.x, targetPose.pose.position.y);
  ...
  state = MOVE;
  if (drawing_enabled)
    drawTarget(targetPose.pose.position, targetPose.header.frame_id);
  return true;
}

void Navigator::setObstaclesFromMessage(
  ...
  for (auto msg : obstacles_msg.obstacles) {
    ...
    if (drawing_enabled) {
      drawObstacle(topPosition, msg.height, msg.radius, msg.social_margin,
                   relevant);
...
void Navigator::stop() {
  // RCLCPP_INFO(this->get_logger(), "Asked to stop %d %d", state,
  // as_.isActive());
  if (state != IDLE) {
    RCLCPP_INFO(this->get_logger(), "Will brake");
    state = IDLE;
...
bool Navigator::is_at_target_point() {
  ...
  // RCLCPP_INFO(this->get_logger(), "is_at_target_point? %.2f %.2f",
  // targetDistance, minDeltaDistance);
...
void Navigator::setMotorVelocity(double newVelocityX, double newVelocityY,
                                 double newVelocityZ, double newAngularSpeed) {
...
void Navigator::setMotorVelocity(double newSpeed, double newVelocityZ,
...
void Navigator::update() {
  ...
  if (state == MOVE) {
    ...
    // RCLCPP_INFO(this->get_logger(), "-> %.3f %.3f", agent->desiredSpeed,
...
Navigator::on_set_parameters(const std::vector<rclcpp::Parameter> &parameters) {
  ...
  for (const auto &param : parameters) {
    ...
    if (param.get_name() == "behavior") {
      ...
      /// TODO(old): distinguish between these two that are alternative in the old
      /// code
...
Navigator::Navigator() : Node("navigator") {
  ...
  cmdStampedPublisher =
      create_publisher<geometry_msgs::msg::TwistStamped>("cmd_vel_stamped", 1);
  stopSubscriber = create_subscription<std_msgs::msg::Empty>(
      "stop", 1, std::bind(&Navigator::setStopFromMessage, this, _1));
  targetPoseSubscriber = create_subscription<geometry_msgs::msg::PoseStamped>(
      "target_pose", 1,
      std::bind(&Navigator::setTargetFromPoseMessage, this, _1));
  targetPointSubscriber = create_subscription<geometry_msgs::msg::PointStamped>(
      "target_point", 1,
      std::bind(&Navigator::setTargetFromPointMessage, this, _1));
  obstaclesSubscriber = create_subscription<hl_navigation_msgs::msg::Obstacles>(
      "obstacles", 1, std::bind(&Navigator::setObstaclesFromMessage, this, _1));
  odom_frame = "";
  odometrySubscriber = create_subscription<nav_msgs::msg::Odometry>(
      "odom", 1, std::bind(&Navigator::setOdometryFromMessage, this, _1));
  ...
  action_server_ = rclcpp_action::create_server<GoToTarget>(
      this, "go_to_target", std::bind(&Navigator::handle_goal, this, _1, _2),
  ...
  param_callback_handle = add_on_set_parameters_callback(
      std::bind(&Navigator::on_set_parameters, this, _1));
  ...
  timer_ =
      create_wall_timer(std::chrono::milliseconds((unsigned)(1e3 * updatePeriod)),
                        std::bind(&Navigator::update, this));
}

rclcpp_action::GoalResponse
Navigator::handle_goal(const rclcpp_action::GoalUUID &uuid,
...
void Navigator::handle_accepted(
    const std::shared_ptr<GoalHandleGoToTarget> _goal_handle) {
