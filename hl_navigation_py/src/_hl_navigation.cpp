...
PYBIND11_MODULE(_hl_navigation, m) {
    ...
    py::class_<Pose2>(m, "Pose2")
        .def(py::init<Vector2, float>(),
             py::arg("position"), py::arg("orientation") = 0.0f)
    ...
    py::class_<Kinematic, std::shared_ptr<Kinematic>>(m, "Kinematic")
        .def_property("max_speed", &Kinematic::get_max_speed, &Kinematic::set_max_speed)
    ...
    py::class_<Holonomic, Kinematic, std::shared_ptr<Holonomic>>(m, "Holonomic")
        .def(py::init<float, float>(), py::arg("max_speed"), py::arg("max_angular_speed"));
    ...
    py::class_<FourWheeled, Wheeled, std::shared_ptr<FourWheeled>>(m, "FourWheeled")
        .def(py::init<float, float>(), py::arg("max_speed"), py::arg("axis"));
    ...
    py::class_<Behavior, std::shared_ptr<Behavior>>(m, "Behavior")
        ...
        .def_property("rotation_tau", &Behavior::get_rotation_tau, &Behavior::set_rotation_tau)
        ...
        .def_property("position", &Behavior::get_position, &Behavior::set_position)
        .def_property("orientation", &Behavior::get_orientation, &Behavior::set_orientation)
        .def_property("twist", [](const Behavior & self){ return self.get_twist();},
                      &Behavior::set_twist)
        .def("get_twist", &Behavior::get_twist, py::arg("relative") = false)
        .def_property("velocity", [](const Behavior & self){ return self.get_velocity();},
        ...
        .def_property("angular_speed", &Behavior::get_angular_speed, &Behavior::set_angular_speed)
        .def_property("wheel_speeds", &Behavior::get_wheel_speeds, &Behavior::set_wheel_speeds)
        .def_property("actuated_twist",
                      [](const Behavior & self){ return self.get_actuated_twist();},
                      &Behavior::set_actuated_twist)
        .def("get_actuated_twist", &Behavior::get_actuated_twist, py::arg("relative") = false)
        .def_property("velocity", [](const Behavior & self){ return self.get_velocity();},
                      [](Behavior & self, const Vector2 v){ return self.set_velocity(v);})
        .def_property("actuated_wheel_speeds", &Behavior::get_actuated_wheel_speeds, nullptr)
        .def("actuate", py::overload_cast<const Twist2 &, float>(&Behavior::actuate))
        ...
        .def("cmd_twist",
             py::overload_cast<float, bool, Behavior::Mode, bool>(&Behavior::cmd_twist),
             py::arg("time_step"), py::arg("relative"),
             py::arg("mode") = Behavior::Mode::move, py::arg("set_as_actuated") = true)
    ...
    py::class_<HLBehavior, Behavior, std::shared_ptr<HLBehavior>>(m, "HLBehavior")
        .def(py::init<std::shared_ptr<Kinematic>, float>(), py::arg("kinematic"), py::arg("radius"))
        .def_property("eta", &HLBehavior::get_eta, &HLBehavior::set_eta)
        .def_property("tau", &HLBehavior::get_tau, &HLBehavior::set_tau)
        .def_property("aperture", &HLBehavior::get_aperture, &HLBehavior::set_aperture)
        .def_property("resolution", &HLBehavior::get_resolution, &HLBehavior::set_resolution)
        .def_property("angular_resolution", &HLBehavior::get_angular_resolution, nullptr)
        .def_property("collision_distances", &HLBehavior::get_collision_distance, nullptr);

    py::class_<ORCABehavior, Behavior, std::shared_ptr<ORCABehavior>>(m, "ORCABehavior")
        .def(py::init<std::shared_ptr<Kinematic>, float>(), py::arg("kinematic"), py::arg("radius"))
        .def_property("time_horizon", &ORCABehavior::get_time_horizon,
                      &ORCABehavior::set_time_horizon)
        .def_property("is_using_effective_center", &ORCABehavior::is_using_effective_center,
                      &ORCABehavior::should_use_effective_center);

    py::class_<HRVOBehavior, Behavior, std::shared_ptr<HRVOBehavior>>(m, "HRVOBehavior")
        .def(py::init<std::shared_ptr<Kinematic>, float>(),
             py::arg("kinematic"), py::arg("radius"));

    py::class_<DummyBehavior, Behavior, std::shared_ptr<DummyBehavior>>(m, "DummyBehavior")
        .def(py::init<std::shared_ptr<Kinematic>, float>(),
             py::arg("kinematic"), py::arg("radius"));
    ...
    py::class_<Controller>(m, "Controller")
        ...
        .def_property("behavior", &Controller::get_behavior, &Controller::set_behavior)
