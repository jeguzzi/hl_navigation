...
class PyBehavior : public Behavior, virtual public PyHasRegister<Behavior> {
 public:
  ...
  void set_kinematic_py(const py::object &value) {
    py_kinematic = value;
    set_kinematic(value.cast<std::shared_ptr<Kinematic>>());
  }
};

class PyKinematic : public Kinematic, virtual public PyHasRegister<Kinematic> {
 public:
  /* Inherit the constructors */
  using Kinematic::Kinematic;
  using Native = Kinematic;

  /* Trampolines (need one for each virtual function) */
  Twist2 feasible(const Twist2 &twist) const override {
    PYBIND11_OVERRIDE_PURE(Twist2, Kinematic, feasible, twist);
  }
  bool is_wheeled() const override {
    PYBIND11_OVERRIDE_PURE(bool, Kinematic, is_wheeled);
  }
  unsigned dof() const override {
    PYBIND11_OVERRIDE_PURE(bool, Kinematic, dof);
  }

  float get_max_angular_speed() const override {
    PYBIND11_OVERRIDE(float, Kinematic, get_max_angular_speed);
  }
...
PYBIND11_MODULE(_hl_navigation, m) {
  ...
  declare_register<Kinematic>(m, "Kinematic");
  ...
  py::class_<HasProperties, std::shared_ptr<HasProperties>>(m, "HasProperties")
      .def("get", &HasProperties::get)
      .def("set", &HasProperties::set)
  ...
  py::class_<Kinematic, PyKinematic, HasRegister<Kinematic>,
             std::shared_ptr<Kinematic>>(m, "Kinematic")
      ...
      .def_property(
          "type", [](Kinematic *obj) { return obj->get_type(); }, nullptr)
  ...
  py::class_<Behavior, PyBehavior, HasRegister<Behavior>, HasProperties,
      ...
      .def(py::init<std::shared_ptr<Kinematic>, float>(), py::arg("kinematic"),
           py::arg("radius"))
      .def_property(
          "kinematic", &Behavior::get_kinematic,
          py::cpp_function(&Behavior::set_kinematic, py::keep_alive<1, 2>()))
  ...
  m.def("load_behavior", &YAML::load_py<PyBehavior>);
  m.def("load_kinematic", &YAML::load_py<PyKinematic>);
  m.def("dump", &YAML::dump<Behavior>);
  m.def("dump", &YAML::dump<Kinematic>);
